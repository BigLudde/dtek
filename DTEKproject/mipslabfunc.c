...
void display_update2(int line) {
	...
	for(i = 0; i < 4; i++) {
		...
		for(j = 0; j < 16; j++) {
			...
			for(k = 0; k < 8; k++)
        if(i == line) spi_send_recv(~font[c*8 + k]);
...
void Name_change(char* name)
{
  ...
  char choice[] = "ABCDEFGHIJKLMNOPQRSTUVXYZ";
  char lettr[] = " ";
  char warning[] = "Limit reached";
  char empty[] = " ";
  int tmp = 0;

  while (1)
  {
    int switchV = getsw(); //initializerar knappar + switches
    int buttons = getbtns();

    if(switchV == 8){
      while(switchV == 8) switchV = getsw();
      display_clear();
      Show_Led(0);
      return;
    }
    ...
    { //visar sliden/bokstaven som kan väljas
      lettr[0] = choice[current];
      display_string(1, lettr);
    ...
    if (buttons == 2)
    { //Tar bokstavsvärdet sliden är på och lägger i txt
      name[tmp] = lettr[0];
      tmp++;
      Show_Led(tmp);
      if (tmp == 8)
